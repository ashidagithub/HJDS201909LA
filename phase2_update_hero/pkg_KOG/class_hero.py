...
class Hero():
    ...
    def __init__(self):
        ''' 初始化一个英雄的函数 ，并随机生成英雄的各类属性'''
        # 生成英雄姓名与定位
        self.__position = random.choice(GLV.TYPE_OF_HERO)     # position
        self.__name = self.__set_name(self.__position)               # name
        # 生成英雄的生命力（体力）与物理攻防数值
        self.life_force = self.__set_life_force(self.__position)
        self.physical_attack = self.__set_physical_attack(self.__position)
        self.physical_defense = self.__set_physical_defense(self.__position)
        # 生成英雄的法力与法术攻防数值
        self.mana_power = self.__set_mana_power(self.__position)
        self.mana_attack = self.__set_mana_attack(self.__position)
        self.mana_defense = self.__set_mana_defense(self.__position)
        # 生成英雄的移动速度
        self.move_speed = self.__set_move_speed(self.__position)
        return
    ...
    def show_me(self):
        # print('-------------------------------------------------------')
        print('(%s)%s:\t' %
              (self.__position, self.__name), end='')
    ...
    def __set_name(self, hero_type):
        ...
        if hero_type == GLV.TYPE_OF_HERO[0]:
            hero_list = (
                '猪八戒', '嫦娥', '孙策', '梦奇', '苏烈', '铠', '东皇太一', '太乙真人',
                '张飞', '牛魔', '刘邦', '程咬金', '关羽', '项羽', '达摩', '夏侯惇'
            )
        elif hero_type == GLV.TYPE_OF_HERO[1]:
            hero_list = (
                '曜', '盘古', '李信', '孙策', '狂铁', '裴擒虎', '苏烈', '铠', '哪吒',
                '雅典娜', '杨戬', '钟馗', '刘备', '孙悟空', '亚瑟', '橘右京', '花木兰',
                '韩信', '露娜', '程咬金', '关羽', '老夫子', '达摩', '李白', '宫本武藏',
                '典韦', '曹操', '夏侯惇', '吕布', '钟无艳', '赵云'
            )
        elif hero_type == GLV.TYPE_OF_HERO[2]:
            hero_list = (
                '云中君', '上官婉儿', '李信', '司马懿', '元歌', '裴擒虎', '百里玄策',
                '百里守约', '孙悟空', '橘右京', '娜可露露', '不知火舞', '花木兰',
                '兰陵王', '韩信', '貂蝉', '李白', '阿轲', '赵云'
            )
        elif hero_type == GLV.TYPE_OF_HERO[3]:
            hero_list = (
                '嫦娥', '上官婉儿', '沈梦溪', '司马懿', '米莱狄', '弈星', '杨玉环',
                '女娲', '梦奇', '干将莫邪', '诸葛亮', '钟馗', '不知火舞', '张良',
                '王昭君', '姜子牙', '露娜', '安琪拉', '貂蝉', '武则天', '甄姬',
                '周瑜', '芈月', '扁鹊', '孙膑', '高渐离', '嬴政', '妲己', '墨子', '小乔'
            )
        elif hero_type == GLV.TYPE_OF_HERO[4]:
            hero_list = (
                '伽罗', '公孙离', '百里守约', '黄忠', '成吉思汗', '虞姬', '李元芳',
